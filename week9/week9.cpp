...
int prev_cam_seq_num = 0;

int motor_address=wiringPiI2CSetup (0x56); 

bool run_program = true;

bool autonomus_yaw = false; 
...
void PIDcontroller()
{
    ...
    if (autonomus_yaw && camera_data.sequence_num != prev_cam_seq_num)
    {
      desired_yaw_speed = -camera_data.yaw*P_AUTO_YAW;
      float desired_roll_auto = camera_data.y*P_Y;
      desired_roll = desired_roll*0.5 + desired_roll_auto*0.5;
      float desired_pitch_auto = camera_data.x*P_X;
    ...
    //   desired_yaw_speed = -camera_data.yaw*P_AUTO_YAW;
    // //   printf("%f,%f\n",time_now,camera_data.yaw);
    // desired_roll = desired_roll*0.5 + whatever
    // }
    // else
    // {
    //   desired_yaw_speed =  yaw_mul*YAW_AMP;
