...
#define JOYSTICK_AUTO_BUTTON key3 //X
...
#define THRUST_NEUTRAL 1575.0
#define THRUST_AMP     300.0
...
float desired_roll_auto = 0.0;
float desired_pitch_auto = 0.0;

int previous_seq_num = -1;
int t_cam = 0.0;
...
int main (int argc, char *argv[])
{
    ...
    while(run_program)
    {
        ...
        // printf("%f,%d\n",time_now,camera_data.yaw);
        // printf("Cam X: %f\tCam Y: %f\n", cam_x_estimated,cam_y_estimated);
        // printf("Desired Roll: %f\tDesired Pitch: %f\n",desired_roll_auto,desired_pitch_auto);
...
void checkInputs()
{
  ...
  if (joystick_data.JOYSTICK_AUTO_BUTTON && !prevKeyValue)
  {
    prevKeyValue = joystick_data.JOYSTICK_AUTO_BUTTON;
    autonomus = !autonomus;
    if (autonomus)
    {
...
void PIDcontroller()
{
    ...
    if (autonomus && camera_data.sequence_num != previous_seq_num)
    {
      desired_yaw_speed = -camera_data.yaw*P_AUTO_YAW;
      float dt = te.tv_nsec - t_cam;
      if(dt<=0)
      {
        dt+=1000000000;
      }
      float dt_sec = dt / 1000000000;
      cam_y_estimated = 0.9*cam_y_estimated + 0.1*camera_data.y;
      cam_x_estimated = 0.9*cam_x_estimated + 0.1*camera_data.x;
      printf("Cam X: %f\tCam Y: %f\n",dt_sec,cam_x_estimated,cam_y_estimated);
      // printf("dt_sec: %f\tDesired Roll: %f\tDesired Pitch: %f\n",dt_sec,desired_roll_auto,desired_pitch_auto);
      desired_roll_auto = camera_data.y*P_Y + D_Y*(cam_y_estimated - previous_y)/dt_sec;
      desired_roll = desired_roll*0.5 + desired_roll_auto*0.5;
      desired_pitch_auto = camera_data.x*P_X + D_X*(cam_x_estimated - previous_x)/dt_sec;
      desired_pitch = desired_pitch*0.5 + desired_pitch_auto*0.5;
      printf("dt_sec: %f\tDesired Roll: %f\tDesired Pitch: %f\n",dt_sec,desired_roll_auto,desired_pitch_auto);
      previous_x = cam_x_estimated;
      previous_y = cam_y_estimated;
      previous_seq_num = camera_data.sequence_num;
      t_cam = te.tv_nsec;
    }
    else
    {
